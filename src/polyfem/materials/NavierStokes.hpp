...
{
	struct NavierStokesExpr;

	struct NavierStokes
	{
		using ExprType = NavierStokesExpr;
	...
	struct NavierStokesExpr
	{
		...
		static NavierStokesExpr from_json(const json &j, const Units &units, const std::string &root_path);
		/// Evaluate material expression at position (x,y,z), time t, and element id.
		NavierStokes eval_expr(double x, double y, double z = 0, double t = 0, int element_id = -1) const;
	};
} // namespace polyfem::material
