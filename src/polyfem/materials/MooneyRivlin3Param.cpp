...
{
	MooneyRivlin3ParamExpr MooneyRivlin3ParamExpr::from_json(const json &j, const Units &units, const std::string &root_path)
	{
		const std::string stress = units.stress();
		MooneyRivlin3ParamExpr out;
	...
	MooneyRivlin3Param MooneyRivlin3ParamExpr::eval_expr(double x, double y, double z, double t, int element_id) const
	{
		MooneyRivlin3Param out{};
