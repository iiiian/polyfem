...
{
	struct VolumePenaltyExpr;

	struct VolumePenalty
	{
		using ExprType = VolumePenaltyExpr;
	...
	struct VolumePenaltyExpr
	{
		...
		static VolumePenaltyExpr from_json(const json &j, const Units &units, const std::string &root_path);
		/// Evaluate material expression at position (x,y,z), time t, and element id.
		VolumePenalty eval_expr(double x, double y, double z = 0, double t = 0, int element_id = -1) const;
	};
} // namespace polyfem::material
