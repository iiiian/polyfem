...
{
	struct IncompressibleLinearElasticityExpr;

	struct IncompressibleLinearElasticity
	{
		using ExprType = IncompressibleLinearElasticityExpr;
	...
	struct IncompressibleLinearElasticityExpr
	{
		...
		static IncompressibleLinearElasticityExpr from_json(const json &j, const Units &units, const std::string &root_path);
		/// Evaluate material expression at position (x,y,z), time t, and element id.
		IncompressibleLinearElasticity eval_expr(double x, double y, double z = 0, double t = 0, int element_id = -1) const;
	};
} // namespace polyfem::material
