...
namespace polyfem::assembler
{
	...
	void ViscousDampingPrev::assemble_gradient(const NonLinearElementAssemblyData &data, span<double> local_gradient) const
	{
		...
		Eigen::MatrixXd local_disp(data.basis_num, size());
		Eigen::MatrixXd local_prev_disp(data.basis_num, size());
		if (size() == 2)
		{
			data.gather_unknowns<2>(local_disp);
			data.gather_previous_unknowns<2>(local_prev_disp);
		}
		else
		{
			data.gather_unknowns<3>(local_disp);
			data.gather_previous_unknowns<3>(local_prev_disp);
		}

		Eigen::MatrixXd local_vel = (local_disp - local_prev_disp) / data.dt;

		Eigen::MatrixXd G;
		G.setZero(data.basis_num, size());
		...
		{
			Eigen::MatrixXd grad(data.basis_num, size());

			if (size() == 2)
				data.gather_basis_grads<2>(p, grad);
			else
				data.gather_basis_grads<3>(p, grad);

			delF_delU = grad * data.jacobian_inverse_transpose(p);
			...
			G -= delF_delU * dRdFdot * (data.weighted_measure[p] / data.dt);
		}

		G.transposeInPlace();

		assert(G.size() == local_gradient.size());
		std::copy(G.data(), G.data() + G.size(), local_gradient.data());
	}

	// E := 0.5(F^T F - I), Compute Stress = \int dF/dt * (2\psi dE/dt + \phi Tr(dE/dt) I) : gradv du
	void ViscousDamping::assemble_gradient(const NonLinearElementAssemblyData &data, span<double> local_gradient) const
	{
		if (data.x_prev.size() != data.x.size())
		{
			std::fill(local_gradient.begin(), local_gradient.end(), 0.0);
		...
		for (long p = 0; p < n_pts; ++p)
		{
			...
			Eigen::MatrixXd delF_delU = grad * data.jacobian_inverse_transpose(p);
			...
			Eigen::MatrixXd tmp = (2 * damping_params_[0]) * dEdt + (damping_params_[1] * dEdt.trace()) * Eigen::MatrixXd::Identity(size(), size());
			G += delF_delU * ((dFdt + def_grad / data.dt) * tmp).transpose() * data.weighted_measure[p];
		}
	...
	void ViscousDamping::assemble_hessian(const NonLinearElementAssemblyData &data, span<double> local_hessian) const
	{
		int local_matrix_size = data.basis_num * size();
		assert(local_hessian.size() == local_matrix_size * local_matrix_size);
		Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> hessian(
		...
		for (long p = 0; p < n_pts; ++p)
		{
			...
			hessian += delF_delU_tensor.transpose() * hessian_temp * delF_delU_tensor * data.weighted_measure[p];
		}

	}

	// Compute \int grad phi_i : dStress / dF^{n-1} : grad phi_j dx
	void ViscousDampingPrev::assemble_hessian(const NonLinearElementAssemblyData &data, span<double> local_hessian) const
	{
		int local_matrix_size = data.basis_num * size();
		assert(local_hessian.size() == local_matrix_size * local_matrix_size);
		Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> stress_grad_Ut(
			local_hessian.data(), local_matrix_size, local_matrix_size);
		stress_grad_Ut.setZero();
		if (data.x_prev.size() != data.x.size())
			return;
		...
		for (long p = 0; p < n_pts; ++p)
		{
			...
			stress_grad_Ut += delF_delU_tensor.transpose() * stress_grad_Ut_temp * delF_delU_tensor * data.weighted_measure[p];
		}

	}

	// E := 0.5(F^T F - I), Compute Energy = \int \psi \| \frac{\partial E}{\partial t} \|^2 + 0.5 \phi (Tr(\frac{\partial E}{\partial t}))^2 du
	double ViscousDamping::compute_energy(const NonLinearElementAssemblyData &data) const
	{
		if (data.x_prev.size() != data.x.size())
			return 0;
		...
		for (long p = 0; p < n_pts; ++p)
		{
			...
			const Eigen::MatrixXd delF_delU = grad * data.jacobian_inverse_transpose(p);
			def_grad = local_disp.transpose() * delF_delU + Eigen::MatrixXd::Identity(size(), size());
			dFdt = local_vel.transpose() * delF_delU;
